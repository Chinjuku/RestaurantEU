            ...
            @foreach ($menus as $item)
                @if ($item->category_id == 1)
                    <button wire:click="toggleModal2({{ $item->menu_id }})"
                        ...
                        <img class="phone:h-[130px] phone:w-[133px] tablet:w-[225px] tablet:h-[180px]  bg-cover rounded-lg"
                ...
                <div
                    ...
                    <form action="{{ route('customer.table.choose', $tableid) }}" method="post" class="flex flex-col gap-5 items-center">
                        ...
                        <div class="flex justify-evenly w-full">
                            {{-- ปุ่มปิด --}}
                            <button type="button" wire:click="toggleModal2({{ $menu_id }})" class="bg-lgreen rounded-[32px] tablet:text-[28px] px-7 py-1 mt-2 font-bold text-darkgreen hover:bg-lightcream duration-700">
                                ยกเลิก
                            </button>
