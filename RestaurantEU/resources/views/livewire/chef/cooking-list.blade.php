<div class="h-seceen bg-lightcream">
    ...
    <div class="flex space-x-4 py-[20px] px-[60px]">
        <a href="{{ route('chef.neworder') }}"
            class="text-3xl text-center  font-bold p-5 border-solid border-2 border-black hover:bg-lgreen">ออร์เดอร์ใหม่</a>
        <a href="{{ route('chef.orderdone') }}"
            class="text-3xl text-center font-bold p-5 border-solid border-2 border-black hover:bg-lgreen">ออร์เดอร์เสร็จสิ้น</a>
    </div>
        ...
        <div class="w-[48%] h-[1000px] bg-cream rounded-[32px] flex flex-col items-center gap-[20px] py-[2%]">
            @foreach ($getorder as $item)
                @if ($item->status === 'cooking')
                    {{-- <a href="{{ route('chef.neworder.show', ['id' => $item->order_id]) }}" --}}
                    <button type="button" wire:click="showOrder({{ $item->order_id }})"
                        class="text-4xl bg-lgreen w-[90%] border border-black flex justify-between p-6">
                ...
                @else
                    {{-- <a href="{{ route('chef.neworder.show', ['id' => $item->order_id]) }}" --}}
                    <button type="button" wire:click="showOrder({{ $item->order_id }})"
                        class="text-4xl w-[90%] border border-black flex justify-between p-6">
                        <p>โต๊ะที่ {{ $item->table_id }}</p>
                        <p>เวลา {{ $item->formattedOrderTime }}</p>
                    </button>
                    {{-- </a> --}}
        ...
        <div class="w-[48%] h-[1000px] bg-cream rounded-[32px]">
            ...
            @if ($open == true)
                <div class="text-4xl pt-[50px] px-[10%]">
                    ...
                    <hr class="h-[2px] w-[700px] bg-black mx-auto mt-[10px] laptop:w-[500px]">
                </div>
                ...
                <div class="overflow-y-scroll overflow-x-hidden m-[4%] min-h-[60%] max-h-[60%]">
                    ...
                    @foreach ($getlists as $index => $list)
                        <div class="flex justify-between px-[10%] mt-[1%] mb-[4%]">
                            ...
                            <div class="flex justify-between space-x-[140px]">
                                ...
                                <div>
                                    <button>
                                        <svg width="40" height="40" class="laptop:w-[55px] laptop:h-[55px]"
                                            viewBox="0 0 60 60" fill="none" xmlns="http://www.w3.org/2000/svg">
                    ...
                    <div class="flex justify-center mt-[5%]">
                        <div class="flex justify-center">
                            <button class="bg-darkgreen text-3xl text-cream font-bold px-5 py-4">เสร็จสิ้น</button>
                        </div>
                    </div>
                @else
                    <div></div>
